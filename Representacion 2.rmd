...
gg.gauge <- function(pos,breaks=c(0,30,50,70,90,100)) {
  ...
  ggplot()+ 
    ...
    geom_polygon(data=get.poly(breaks[2],breaks[3]),aes(x,y),fill="#fdae61")+
    ...
    geom_polygon(data=get.poly(breaks[5],breaks[6]),aes(x,y),fill="#1a9641")+
    geom_polygon(data=get.poly(pos-1,pos+1,0.2),aes(x,y))+
    geom_text(data=as.data.frame(breaks), size=5, fontface="bold", vjust=0, 
              aes(x=1.1*cos(pi*(1-breaks/100)),y=1.1*sin(pi*(1-breaks/100)),label=c("0", "D", "C", "B (Target)", "A", "A+")))+
